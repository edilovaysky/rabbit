...
geometryParticles.addAttribute(
  "position",
...
geometryParticlesBright.addAttribute(
  "position",
  new THREE.Float32BufferAttribute(vertices, 3)
);
var particlesBright = new THREE.Points(
  geometryParticlesBright,
  new THREE.PointsMaterial({
    //color: 0xf0f0f0
...
for (var i = 0; i < geometrySphere.faces.length; i++) {
  geometrySphere.faces[i].color.setRGB(
    Math.random(),
    Math.random(),
    Math.random()
  );
